...
int main(int argc, char *argv[]){
  ...
  if(is_pp){
    trkCorr = new TrkCorr("TrkCorr_July22_Iterative_pp_eta2p4/");
  
  }else{
    trkCorr = new TrkCorr("TrkCorr_Jun7_Iterative_PbPb_etaLT2p4/");
  ...
  for(int fi = 0; fi < (int) file_names.size(); fi++) {
    ...
    if(is_data&&!is_pp){
      me_file_name = Form("/data/htrauger/PbPb_MinBias_skims_8_7_16/Data2015_%d.root",parti%4+1);
    }else if( parti< (int) file_names.size()-1){
    ...
    int meptrig = 20;  //note:  this is mixing events per event in which at least one jet (reco or gen in MC) passes selection.
    
    gRandom->SetSeed(0);
    Long64_t me, me_place;
    ...
    int jet_cent, jet_vzbin, me_passes, vzbin,centbin;
    vector <Int_t> me_cent;
    vector <Int_t> me_vzbin;
    vector <Int_t> me_evt_sel;

    vector <Int_t> me_bins[31][201];
	...
	if((is_pp&&is_data && me_tree->pBeamScrapingFilter==1 && me_tree->pPAprimaryVertexFilter==1&&me_tree->HBHENoiseFilterResultRun2Loose==1) || 
	   (!is_pp&&is_data  && me_tree->pprimaryVertexFilter==1 && me_tree->pcollisionEventSelection==1&&me_tree->HBHENoiseFilterResultRun2Loose==1) ||
	   ( !is_data)){
	  me_passes = 1;
	}else{
	  continue;
	...
	if(!is_pp){  
	  centbin = centbins->FindBin(me_tree->hiBin);
	  me_cent.push_back(centbin);
	}else{
	  centbin = 0;
	}
	me_evt_sel.push_back(me_passes);

	if(vzbins->FindBin(me_tree->vz)>30){continue;}
	vzbin = (vzbins->FindBin(me_tree->vz));
	
	me_vzbin.push_back(vzbin);

	me_bins[vzbin][centbin].push_back(mei);
	...
	if(do_mixing&&foundjet){ // new scheme:  match event once if we have found a jet
	  ...
	  while(mevi< meptrig){  //
	    ...
	    if(me_bins[jet_vzbin][jet_cent].size()==0)continue;
	  
	    me_place = gRandom->Rndm()*(me_bins[jet_vzbin][jet_cent].size()-1);

	    //    cout<<me_place<<" "<<me_bins[jet_vzbin][jet_cent].size()<<endl;

	    me = me_bins[jet_vzbin][jet_cent].at(me_place);
	    ...
	    for(int j4i = 0; j4i < (int) my_primary->calo_jtpt->size(); j4i++) {
	      ...
	      //cout<<"did jet selection for mixing"<<endl;

	      for(int tracks =0; tracks < (int) me_tree->trkPt->size(); tracks++){

		//cout<<me_tree->trkPt->size()<<" "<<tracks<<endl;
		
		if(fabs(me_tree->trkEta->at(tracks))>=trketamaxcut) continue;
		if (me_tree->highPurity->at(tracks)!=1) continue;
		if(!is_pp){
		 		  
		  if ((me_tree->trkChi2->at(tracks)/(me_tree->trkNdof->at(tracks))/(me_tree->trkNlayer->at(tracks)) > 0.15)) continue;
		  if (me_tree->trkNHit->at(tracks)< 11 && (me_tree->trkPt->at(tracks)> 0.7)) continue;
		
		  
		}
		float Et = (me_tree->pfHcal->at(tracks)+me_tree->pfEcal->at(tracks))/TMath::CosH(me_tree->trkEta->at(tracks));
	   
		if(!(me_tree->trkPt->at(tracks)<20 || (Et>0.5*me_tree->trkPt->at(tracks)))) continue;
		...
		//cout<<"did selections for "<<j4i<<endl;
