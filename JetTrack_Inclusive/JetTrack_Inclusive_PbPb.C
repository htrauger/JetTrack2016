...
int main(int argc, char *argv[]){
  ...
  for(int fi = 0; fi < (int) file_names.size(); fi++) {
    ...
    mixing_tree_PbPb *my_primary = new mixing_tree_PbPb(inp_tree);
    ...
    mixing_tree_PbPb *me_tree = new mixing_tree_PbPb(inp_tree2);
	...
	me_vzbin.push_back((vzbins->FindBin(me_tree->vz)));
	if(vzbins->FindBin(me_tree->vz)>31){cout<<"THIS IS A PROBLEM!!"<<endl;}
      }
	...
	if(is_pp){
	  ...
	  //THESE WERE APPLIED ON THE SKIM
	  /*
	...
	for(int j4i = 0; j4i < (int) my_primary->jtpt->size(); j4i++) {
	  ...
	  is_inclusive = kFALSE;
	  if( my_primary->trackMax->at(j4i)/my_primary->rawpt->at(j4i) > 0.98 ||my_primary->trackMax->at(j4i)/my_primary->rawpt->at(j4i) < 0.01) continue;
	  if( fabs(my_primary->jteta->at(j4i)) > etacut ) continue;
	  //  if( my_primary->jtpt->at(j4i) > pTmaxcut ) continue;
	  if( my_primary->jtpt->at(j4i) > pTmincut ){
	  ...
	  if(!is_data){
		...
		cout<<"No gen jet found: ("<<jet_dir_eta<<","<<jet_dir_phi<<") pT = "<<my_primary->jtpt->at(j4i)<<endl;
		unmatched_counter++;
	  ...
	  if(is_inclusive == kTRUE){
	    my_hists[data_mc_type_code]->all_jets_corrpT[ibin][ibin2]->Fill(my_primary->jtpt->at(j4i), wvz*wcen); 
	...
	if(do_mixing&&foundjet){ // new scheme:  match event once if we have found a jet
	  ...
	  while(mevi< meptrig &&startovercheck <2){  //
	    ...
	    for(int j4i = 0; j4i < (int) my_primary->jtpt->size(); j4i++) {
	      ...
	      if( fabs(my_primary->jteta->at(j4i)) > etacut ) continue;
	      //  if( my_primary->jtpt->at(j4i) > pTmaxcut ) continue;
	      if( my_primary->jtpt->at(j4i) < pTmincut) continue;
