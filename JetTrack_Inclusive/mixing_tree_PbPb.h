...
#ifndef mixing_tree_PbPb_h
#define mixing_tree_PbPb_h
...
TBranch        *b_jteta;   //!
 ...
 TBranch        *b_pthat;   //!
  ...
  mixing_tree_PbPb(TTree *tree=0);
  virtual ~mixing_tree_PbPb();
...
#ifdef mixing_tree_PbPb_cxx
mixing_tree_PbPb::mixing_tree_PbPb(TTree *tree) : fChain(0) 
{
  ...
  if (tree == 0) {
    ...
    f->GetObject("mixing_tree_PbPb",tree);

  }
  Init(tree);
}

mixing_tree_PbPb::~mixing_tree_PbPb()
{
  if (!fChain) return;
  delete fChain->GetCurrentFile();
}

Int_t mixing_tree_PbPb::GetEntry(Long64_t entry)
{
  // Read contents of entry.
  if (!fChain) return 0;
  return fChain->GetEntry(entry);
}
Long64_t mixing_tree_PbPb::LoadTree(Long64_t entry)
{
...
void mixing_tree_PbPb::Init(TTree *tree)
{
...
Bool_t mixing_tree_PbPb::Notify()
{
...
void mixing_tree_PbPb::Show(Long64_t entry)
{
...
Int_t mixing_tree_PbPb::Cut(Long64_t entry)
{
...
#endif // #ifdef mixing_tree_PbPb_cxx
