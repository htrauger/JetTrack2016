...
Int_t spill_over_qg_rx(bool quark_gluon = kTRUE, bool rx_plane = kFALSE, bool spilled_in_out = kFALSE, bool is_jff = kFALSE){
  ...
  TString Type_strs[5] = {"Nominal","In","Out","Quark","Gluon"};

  TFile *f_in[5];

  f_in[0] = new TFile("../me_correct/HydJet_RecoJet_GenTrack_NoSube0_Inclusive_Correlations.root");
  if(is_jff) f_in[0] = new TFile("../me_correct/HydJet_RecoJet_GenTrack_Sube0_Inclusive_Correlations.root");
  if(rx_plane) f_in[1] = new TFile("../me_correct/HydJet_RecoJet_GenTrack_NoSube0_Inclusive_Correlations_InRxPlaneOnly.root");
  else  f_in[1] = new TFile("../me_correct/HydJet_RecoJet_GenTrack_NoSube0_Inclusive_Correlations_SpillOutOnly.root");
  if(rx_plane) f_in[2] = new TFile("../me_correct/HydJet_RecoJet_GenTrack_NoSube0_Inclusive_Correlations_OutRxPlaneOnly.root");
  else  f_in[2] = new TFile("../me_correct/HydJet_RecoJet_GenTrack_NoSube0_Inclusive_Correlations_SpillInOnly.root");
  if(is_jff)  f_in[3] = new TFile("../me_correct/HydJet_RecoJet_GenTrack_Sube0_Inclusive_Correlations_QuarkOnly.root");
  else  f_in[3] = new TFile("../me_correct/HydJet_RecoJet_GenTrack_NoSube0_Inclusive_Correlations_QuarkOnly.root");
  if(is_jff)  f_in[4] = new TFile("../me_correct/HydJet_RecoJet_GenTrack_Sube0_Inclusive_Correlations_GluonOnly.root");
  else f_in[4] = new TFile("../me_correct/HydJet_RecoJet_GenTrack_NoSube0_Inclusive_Correlations_GluonOnly.root");
  ...
  for(int i = 0; i<nTrkPtBins-1; i++){
	...
	result[g][i][j] = (TH2D*)f_in[g]->Get((TString)("Raw_Yield_" + CBin_strs[j] + "_" + CBin_strs[j+1] + "_Pt100_Pt1000_" + TrkPtBin_strs[i] + "_" + TrkPtBin_strs[i+1]))->Clone((TString)("Raw_Yield_" + Type_strs[g]+"_"+CBin_strs[j] + "_" + CBin_strs[j+1] + "_Pt100_Pt1000_" + TrkPtBin_strs[i] + "_" + TrkPtBin_strs[i+1]));
	...
	int lbin = result[g][i][j]->GetXaxis()->FindBin(-.299);
	int rbin = result[g][i][j]->GetXaxis()->FindBin(.299);
