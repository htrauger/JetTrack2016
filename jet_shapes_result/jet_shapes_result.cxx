  ...
  */
  ...
  for(int g=0; g<2; g++){
	...
	for (int ibin3=0;ibin3<nTrkPtBins-1;ibin3++){
	  ...
	  if(is_number){
	    if(ibin3==0) result[g][ibin][ibin2][ibin3]->Scale(2.);
	  ...
	  */
	  ...
	  }else if(!is_number&&ibin3>0&&ibin3<9){
	    
	    result[g][ibin][ibin2][9]->Add(result[g][ibin][ibin2][ibin3]);
	 
	  }else if(is_number&&ibin3>0&&ibin3<8){
	...
	*/
	  ...
	  //if(is_number)JetShape2[g][ibin][ibin3]->Divide(JetShape2_geo[g][ibin][ibin3]);
...
TPave *cover_x_b = new TPave(0.9,0.,0.995,0.17);
   ...
   if(is_number){
     ...
     for(int ibin = 0; ibin < nCBins; ibin++){
       ...
       Integral_Pt[0][ibin]->SetMinimum(-1.);
	 ...
	 Integral_Pt[0][ibin]->GetYaxis()->SetTitle("Tracks per IBINet");
       }else{
	 ...
	 int_legend->AddEntry( Integral_Pt[0][ibin],"PbPb integral |#Delta#eta|<1.0, |#Delta#phi|<1.0");
	 int_legend->AddEntry( Integral_Pt[1][0],"pp integral |#Delta#eta|<1.0, |#Delta#phi|<1.0");
	 ...
	 labels->Draw("same");
       ...
       Integral_diff_Pt[0][ibin]->SetMinimum(-0.5);
