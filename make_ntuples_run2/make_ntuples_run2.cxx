...
int main(int argc,char *argv[]){
  int dataset_type_code = atoi(argv[1]); 
  int in_file_min = atoi(argv[2]); 
  int in_file_max = atoi(argv[3]); 
  ...
  for(int in_file_num = in_file_min; in_file_num < in_file_max+1; in_file_num++){
    ...
    if(is_data&&!do_PbPb){
      ...
      in_file_name = Form("/store/user/velicanu/HIHardProbes/HIHardProbes-HIRun2015-PromptReco-v1-FOREST-1-v22/160126_202951/0000/HiForestAOD_%d.root",in_file_num);
    }else if(!is_data&&!do_PbPb){

      if(dataset_type_code!=14)cout<<"WATCH OUT, YOU ARE RUNNING ON PTHAT80"<<endl;

      in_file_name = Form("root://cms-xrd-global.cern.ch///eos/uscms/store/user/htrauger/Pythia8_Dijet%d_pp502_TuneCUETP8M1/Pythia8_Dijet%d_pp502_TuneCUETP8M1_HiForest/160713_235920/0000/HiForestAOD_%d.root",dataset_pthats[dataset_type_code],in_file_num);
    }else{
    ...
    my_file = TFile::Open(in_file_name);
