 ...
 Double_t integral_phi_err[12][nTrkPtBins][nCBins];
  ...
  for(int i = 0; i<nTrkPtBins; i++){
	...
	*/
	...
	jff_residual_dEta[g+2][i][j] = (TH1D*)f_jff_reco->Get(jff_name)->Clone(jff_name);

	jff_name.ReplaceAll("Eta","Phi");
	jff_residual_dPhi[g+2][i][j] = (TH1D*)f_jff_reco->Get(jff_name)->Clone(jff_name);
	 
	jff_name = make_name("JFF_Residual_Eta_",g+10,i,3,l,pTlabel,centlabel,Ajlabel);
	jff_residual_dEta[g][i][j] = (TH1D*)f_jff_reco->Get(jff_name)->Clone(jff_name);

	jff_name.ReplaceAll("Eta","Phi");
	jff_residual_dPhi[g][i][j] = (TH1D*)f_jff_reco->Get(jff_name)->Clone(jff_name);
	}

	signal_dEta_rebin[g+2][i][j]->Add(jff_residual_dEta[g+2][i][0],-1.);
	signal_dPhi_rebin[g+2][i][j]->Add(jff_residual_dPhi[g+2][i][0],-1.);
	...
	spill_over_dEta[g+2][i][j] = (TH1D*)f_spillover->Get(spill_name)->Clone(spill_name);
	spill_name.ReplaceAll("Eta","Phi");
	spill_over_dPhi[g+2][i][j] = (TH1D*)f_spillover->Get(spill_name)->Clone(spill_name);
	...
	spill_over_dEta[g][i][j] = (TH1D*)f_spillover->Get(spill_name)->Clone(spill_name);
	  
	spill_name.ReplaceAll("Eta","Phi");
	spill_over_dPhi[g][i][j] = (TH1D*)f_spillover->Get(spill_name)->Clone(spill_name);

	
	signal_dEta_rebin[g+2][i][j]->Add(spill_over_dEta[g+2][i][j],-1.);
	signal_dPhi_rebin[g+2][i][j]->Add(spill_over_dPhi[g+2][i][j],-1.);
	...
	jff_residual_dEta[g+2][i][j] = (TH1D*)f_jff_gen->Get(jff_name)->Clone(jff_name);
	}
	jff_name.ReplaceAll("Eta","Phi");
	if(i>4)    jff_residual_dPhi[g+2][i][j] = (TH1D*)f_jff_reco->Get(jff_name)->Clone(jff_name);
	else     jff_residual_dPhi[g+2][i][j] = (TH1D*)f_jff_gen->Get(jff_name)->Clone(jff_name);
	    
	jff_name = make_name("JFF_Residual_Eta_",g+10,i,j,l,pTlabel,centlabel,Ajlabel);
	if(i>4)    jff_residual_dEta[g][i][j] = (TH1D*)f_jff_reco->Get(jff_name)->Clone(jff_name);
	else jff_residual_dEta[g][i][j] = (TH1D*)f_jff_gen->Get(jff_name)->Clone(jff_name);

	jff_name.ReplaceAll("Eta","Phi");
	if(i>4)   jff_residual_dPhi[g][i][j] = (TH1D*)f_jff_reco->Get(jff_name)->Clone(jff_name);
	else    jff_residual_dPhi[g][i][j] = (TH1D*)f_jff_gen->Get(jff_name)->Clone(jff_name);
	 
	signal_dEta_rebin[g+2][i][j]->Add(jff_residual_dEta[g+2][i][j],-1.);
	signal_dPhi_rebin[g+2][i][j]->Add(jff_residual_dPhi[g+2][i][j],-1.);
	...
	*/
	...
	legend->AddEntry(signal_dPhi_rebin[0][i][j],"2.76 TeV ak3Calo pp");
	legend->AddEntry(signal_dPhi_rebin[1][i][0],"5.02 TeV ak4Calo pp");
	legend->SetTextSize(0.06);
	legend->SetLineColor(0);
	legend->Draw();
      }
  ...
  for(int j = 0; j < nCBins; j++){
    ...
    Integral_phi_Pt[0][j]->SetMinimum(-1.);
    ...
    Integral_diff_Pt[0][j]->SetMinimum(-0.5);
