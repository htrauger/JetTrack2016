...
Int_t particle_yields_closures_raw(bool is_number = kTRUE, int type_code = 1, int do_closures = 0, int ref_is_reco = 0){
 ...
 TFile *f_spillover = new TFile("../spill_over/Inclusive_Hydjet_SpillOvers.root");
  ...
  for(int i = 0; i<nTrkPtBins; i++){
	...
	else	signal_dPhi[g][i][j] = (TH1D*)result[g][i][j]->ProjectionY((TString)("Proj_dPhi_pp_" + CBin_strs[j] + "_" + CBin_strs[j+1] + TrkPtBin_strs[i] + "_" + TrkPtBin_strs[i+1]),lbin,rbin);
	/*	    
	...
	*/
	...
	else 	signal_dEta[g][i][j] = (TH1D*)result[g][i][j]->ProjectionX((TString)("Proj_dEta_pp_" + CBin_strs[j] + "_" + CBin_strs[j+1]+ TrkPtBin_strs[i] + "_" + TrkPtBin_strs[i+1]),lbin,rbin);
