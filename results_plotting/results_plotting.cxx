...
Int_t results_plotting(bool is_number=0,bool do_ref=kFALSE){
  ...
  for(int g=0; g<2; g++){
	...
	for (int ibin3=0;ibin3<nTrkPtBins;ibin3++){
	  ...
	  if(do_ref)	  JetShape_ref[g+3][ibin][ibin3] = (TH1D*) JetShape_ref[g][ibin][ibin3]->Clone((TString) (stem + "_NormedJetShape_" + CBin_strs[ibin+1] + "_" +TrkPtBin_strs[ibin3]+"_" +TrkPtBin_strs[ibin3+1]));
	  ...
	  if(!is_number&&ibin3==9){
	    ...
	    if(do_ref){
	      ...
	      for(int k = 0; k<10; k++){
		JetShape_ref[g+3][ibin][k]->Scale(1./norm);
	      }
	    }
	  }
	}
  ...
  for (int ibin=0;ibin<nCBins;ibin++){ 
	...
	if(is_number&&g>0)continue;
	  ...
	  if(do_ref){    
	    JetShape_ref_diff[g][ibin][ibin3] = (TH1D*)JetShape_ref[g][ibin][ibin3]->Clone((TString)(diff_name+"_Ref"));
	    JetShape_ref_diff[g][ibin][ibin3]->Add( JetShape_ref[g+1][0][ibin3],-1. );
	  }
  ...
  for(int ibin = 0; ibin<4; ibin++){
    ...
    if(do_ref)    JetShape_ref_ratio[3][ibin][9]->Draw("same");
  ...
  if(do_ref){
    ...
    PAS_plot_3row->SaveAs("JetShapes_WithHighpT_pTweighted_3Row_PreapprovalRef.pdf");
    PAS_plot_3row->SaveAs("JetShapes_WithHighpT_pTweighted_3Row_PreapprovalRef.png");
