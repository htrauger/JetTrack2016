...
Int_t results_plotting2(bool is_number=0,bool do_ref=kFALSE){
  ...
  /*
  float jetshape_y_min = .1;
  float jetshape_y_max = 2000.;

 float number_diff_y_min = -50.;
  float number_diff_y_max = 150.;
  */
 float number_diff_y_min = -1.;
  float number_diff_y_max = 2.;
  ...
  for(int g=0; g<2; g++){
	...
	for (int ibin3=0;ibin3<nTrkPtBins;ibin3++){
	  ...
	  if(g==0){  
	    ...
	    JetShape_syst[g+3][ibin][ibin3] = (TH1D*)f_in->Get((TString)("Jet_Shape_SystErr_"+ CBin_strs[ibin] + "_" + CBin_strs[ibin+1] + "_" + TrkPtBin_strs[ibin3] + "_" + TrkPtBin_strs[ibin3+1]))->Clone((TString) ("Jet_Shape_Syst_Diff2_" + CBin_strs[ibin+1] + "_" +TrkPtBin_strs[ibin3]+"_" +TrkPtBin_strs[ibin3+1]));
	    ...
	    JetShape_syst[g+3][ibin][ibin3]->SetMarkerStyle(20);
	    JetShape_syst[g+3][ibin][ibin3]->SetMarkerSize(1);
	    JetShape_syst[g+3][ibin][ibin3]->SetMarkerColor(kWhite);
	    JetShape_syst[g+3][ibin][ibin3]->SetFillColor(kBlack);
	    JetShape_syst[g+3][ibin][ibin3]->SetFillStyle(3004);
	  ...
	  if(g==1){
	    ...
	    for(int k = 0; k<4; k++){
	      ...
	      JetShape_syst[g+2][k][ibin3]->Add( JetShape_syst[g][0][ibin3],-1.);
	    }
	  }
  ...
  for(int ibin = 0; ibin<4; ibin++){
	...
	JetShape_ref_diff[0][ibin][9]->Draw("same");
      ...
      JetShape_diff[0][ibin][9]->GetXaxis()->SetTitleSize(x_title_size);
      JetShape_diff[0][ibin][9]->GetXaxis()->SetLabelSize(x_label_size);
      JetShape_diff[0][ibin][9]->GetYaxis()->SetTitleSize(x_title_size);
      JetShape_diff[0][ibin][9]->GetYaxis()->SetLabelSize(x_label_size);
  ...
  if(ibin==0){
	...
	Integral_syst_Pt[1][0]->SetMarkerColor(kBlack);
      ...
      Integral_syst_Pt[0][ibin]->SetFillColor(kMagenta-7);
	...
	signal_dEta_PbPb_pp[0][ibin][8]->Draw("same");
      ...
      dummy_axis_diff->SetTitleSize(y_title_size);
      dummy_axis_diff->SetLabelSize(y_label_size);
      ...
      signal_dPhi_stack[1][0]->GetYaxis()->SetLabelSize(y_title_size);
